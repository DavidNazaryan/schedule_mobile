...
class SpaScheduleClient:
    ...
    def fetch_schedule(
        ...
        *,
        ...
        try:
            ...
            soup = self._submit_form()
            html = str(soup)
            group_name = self._current_group_name(soup, group_id)
            ...
            print(f"Ошибка при получении расписания для группы {group_id}: {e}")
            raise
    ...
    def _ensure_initial_state(self) -> BeautifulSoup:
        if self._last_soup is None:
            try:
                ...
                print(f"Ошибка при подключении к {self.base_url}: {e}")
                raise ConnectionError(f"Не удалось подключиться к серверу расписания: {e}")
    ...
    def _submit_form(self) -> BeautifulSoup:
        ...
        try:
            ...
            raise ConnectionError(f"Не удалось получить расписание: {e}")
    ...
    @staticmethod
    def _current_group_name(soup: BeautifulSoup, group_id: Optional[str] = None) -> Optional[str]:
        ...
        if group_id is not None:
            ...
            for candidate in soup.select("#timetableform-groupid option"):
