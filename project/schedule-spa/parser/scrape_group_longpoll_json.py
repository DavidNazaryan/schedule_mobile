...
def select_by_label(value, label_text, page):
    try:
        # Пример автоматического диапазона дат: два месяца до сегодняшнего дня
        faculty_label = "Магистратура"  # замените на нужное значение
        course_label = "1"               # замените на нужное значение
        group_label = "ГМУ-1-2-УЭФ"      # замените на нужное значение
        date_to = D.today()
        date_from = date_to - timedelta(days=60)
        lessons = scrape_group_schedule(faculty_label, course_label, group_label, date_from, date_to)
        print(f"Выгружено занятий: {len(lessons)}")
