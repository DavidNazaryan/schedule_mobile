...
from .roles import role_manager, check_permission, Permission, can_user_manage_group, can_user_view_group_schedule, can_user_edit_group_schedule, UserRole
...
class TelegramUser:
    ...
    language_code: Optional[str] = None  # Новое поле для языка пользователя
    role: UserRole = UserRole.STUDENT
    group_id: Optional[str] = None
    status: str = "active"  # Новое поле для статуса пользователя
    created_at: Optional[str] = None  # Новое поле для даты создания
    last_seen: Optional[str] = None  # Новое поле для последнего входа
    ...
    def verify_telegram_data(self, init_data: str) -> Optional[TelegramUser]:
        ...
        try:
            ...
            user = TelegramUser(
                ...
                language_code=user_data.get('language_code'),  # Новое поле
                ...
                created_at=datetime.now().isoformat(),  # Новое поле
                last_seen=datetime.now().isoformat()  # Новое поле
            )
...
def can_edit_schedule(user: TelegramUser, group_id: str) -> bool:
    ...
    return can_user_edit_group_schedule(user.role, group_id, user.group_id)


def can_view_schedule(user: TelegramUser, group_id: str) -> bool:
    """Проверяет, может ли пользователь просматривать расписание группы"""
    return can_user_view_group_schedule(user.role, group_id, user.group_id)
...
def can_manage_group(user: TelegramUser, group_id: str) -> bool:
    """Проверяет, может ли пользователь управлять конкретной группой"""
    return can_user_manage_group(user.role, group_id, user.group_id)


def has_permission(user: TelegramUser, permission: Permission) -> bool:
    """Проверяет, есть ли у пользователя определенное разрешение"""
    return check_permission(user.role, permission)


def get_user_permissions_dict(user: TelegramUser) -> Dict[str, bool]:
