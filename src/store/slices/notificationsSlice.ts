...
const notificationsSlice = createSlice({
  ...
  reducers: {
    ...
    setNotifications: (state, action: PayloadAction<NotificationItem[]>) => {
      ...
      state.unreadCount = action.payload.filter((item: NotificationItem) => !item.sent).length;
    },
    ...
    markAsRead: (state, action: PayloadAction<number>) => {
      const notification = state.items.find(
        (item: NotificationItem) => item.id === action.payload
      );
    ...
    markAllAsRead: (state) => {
      state.items.forEach((item: NotificationItem) => {
        item.sent = true;
      });
      state.unreadCount = 0;
    },
