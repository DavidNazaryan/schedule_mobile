...
class OfflineService {
  ...
  private getScheduleCacheKey(groupId: string, from?: string, to?: string): string {
    const range = [from, to].filter(Boolean).join('_');
    return range ? `schedule_${groupId}_${range}` : `schedule_${groupId}`;
  }

  // Cache schedule data
  async cacheSchedule(groupId: string, schedule: ScheduleItem[], from?: string, to?: string): Promise<void> {
    const cacheKey = this.getScheduleCacheKey(groupId, from, to);
    await this.cacheData(cacheKey, schedule, 300000); // 5 минут
  }

  // Read schedule data from cache
  async getCachedSchedule(groupId: string, from?: string, to?: string): Promise<ScheduleItem[] | null> {
    const cacheKey = this.getScheduleCacheKey(groupId, from, to);
    return await this.getCachedData(cacheKey);
  }

  async cacheFullSchedule(groupId: string, schedule: ScheduleItem[]): Promise<void> {
    await this.cacheData(`schedule_full_${groupId}`, schedule, 86400000); // 24 часа
  }

  async getFullSchedule(groupId: string): Promise<ScheduleItem[] | null> {
    return await this.getCachedData(`schedule_full_${groupId}`);
  }
